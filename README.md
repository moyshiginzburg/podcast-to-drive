...
- **Filenames:** `YYMMDD <episode title>.mp3`. Large downloads use chunked parts: `YYMMDD <title> (חלק 001).mp3`, etc. (`CHUNK_SIZE` is 40 MB; `UrlFetch` responses are capped at 50 MB).
- **Execution limit:** Long automatic runs save `resumeState` and schedule a one-time continuation trigger; `lastRunTime` is updated when a full run completes successfully.
