...
This Google Apps Script project binds to a **Google Sheet** and provides a **modal manager window** to subscribe to podcasts (iTunes search, **RSS URL**, or **OPML**), browse episodes, download MP3s to **Google Drive** under a folder named `הסכתים`, and optionally run an **automatic download** every 6 hours. A **Log** sheet records activity and includes **links** to downloaded files.

### Project files
| File | Role |
|------|------|
| `Code.gs` | Server-side logic (RSS, Drive, subscriptions, triggers) |
| `Sidebar.html` | Modal UI (Hebrew-facing strings) |
...
6. Save, run **`onOpen`**, approve permissions, refresh the Sheet, open the manager window from the menu
...
| Location | Contents |
|----------|----------|
| Sheet **`מנויים`** | Subscriptions (`כתובת RSS`, `שם`, `תמונה`, `תאריך הרשמה`, `סטטוס`) |
| Sheet **`הורדות`** | Downloaded episode URLs (`כתובת`) |
| Script **Properties** | Resume state / trigger IDs / last successful auto-run timestamp |
...
<h3>חיפוש ב־iTunes, הזנת RSS, OPML ופעולות בחלון הניהול</h3>
<ul>
...
<h3>איפה נשמרים המנויים, כתובות שהורדו, תיקיות ה־MP3 והלוג</h3>
<table>
  <thead>
    <tr>
      <th>מיקום</th>
  ...
  <tbody>
    <tr>
      <td>גיליון <code>מנויים</code></td>
      <td>רשימת מנויים עם סטטוס <strong>פעיל / בוטל</strong></td>
    </tr>
    <tr>
      <td>גיליון <code>הורדות</code></td>
      <td>כתובות פרקים שכבר הורדו</td>
    </tr>
    <tr>
      <td>מאפייני הסקריפט</td>
      <td>מצב המשך ריצה, מזהה טריגר זמני, וזמן הורדה אוטומטית מוצלחת אחרונה</td>
    </tr>
    <tr>
